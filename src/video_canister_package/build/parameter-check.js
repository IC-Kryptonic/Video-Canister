...
function checkUpdateMetadataParams(input) {
    ...
    if (!input.identity || !checkValidIdentity(input.identity))
        parameterError(errorMessage, 'identity');
    if (!input.newName || !checkValidString(input.newName))
        parameterError(errorMessage, 'newName');
    if (!input.newDescription || !checkValidString(input.newDescription))
        parameterError(errorMessage, 'newDescription');
    return input;
}
...
function checkUpdateVideoParams(input) {
    ...
    if (!input.identity || !checkValidIdentity(input.identity))
        parameterError(errorMessage, 'identity');
    if (!input.newChunkNum || !checkValidNumber(input.newChunkNum))
        parameterError(errorMessage, 'newChunkNum');
    if (!input.newVideoBuffer || !checkValidBuffer(input.newVideoBuffer))
        parameterError(errorMessage, 'newVideoBuffer');
    return input;
}
